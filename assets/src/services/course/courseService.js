...
class CourseService {
    ...
    getPublicCourselist({ course_name = '', status, category_id, need_testing, time_start, time_end, page, pagesize}) {
        let url = urlPre + '/lists'
        return api.get(url, { course_name, status, category_id, need_testing, time_start, time_end, page, pagesize }, false).then(ret => {
            if (ret.code == 0) {
                return ret
            } else {
                return Promise.reject(ret)

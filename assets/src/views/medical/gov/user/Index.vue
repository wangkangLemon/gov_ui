        ...
        <el-table class="data-table" v-loading="loadingData" :data="tableData" :fit="true" @select="selectRow" @select-all="selectRow" border>
            ...
            <el-table-column fixed="right" width="207" label="操作">
                <template scope="scope">
                    ...
                    <el-button @click="$router.push({name: 'medical-edit', params: {sysInfo: scope.row, sys_id: scope.row.id}})" type="text" size="small">详情
                        <!--a-->
                    </el-button>
                    <el-button @click="$router.push({name: 'medical-edit', params: {sysInfo: scope.row, sys_id: scope.row.id, sys_type:'edit'}})" type="text" size="small">编辑
                        <!--a-->
                    </el-button>
...
<script>
import userService from '../../../services/medical/userService.js'
        ...
        online(index, row) {
            if(row.deleted == 0){
                xmview.showDialog(`你将要启用管理员<span style="color:red">${row.name}</span> 确认吗?`, () => {
                    userService.online(row.id).then((ret) => {
                        row.disabled = 0
                    })
                })
            }else{
